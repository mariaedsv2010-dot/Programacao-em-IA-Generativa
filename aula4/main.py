...
st.title('ENSINAR A MAQUINA PREVER O FUTURO')
...
if st.button('prever campeão'):
    previsao = modelo_copa.predict([[gols_input, ranking_input]])
    st.write(f'O campeão previsto é: {previsao[0]}')
...
modelos_vendas = LinearRegression()
modelos_vendas.fit(dados_vendas[['investimento']], dados_vendas['faturamento'])
investimento_input = st.number_input('quanto você investiu em marketing?',0,10000,50)
if st.button('Prever faturamento'):
    faturamento_previsto = modelos_vendas.predict([[investimento_input]])
